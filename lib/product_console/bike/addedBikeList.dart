...
import 'package:verify_devices/product_console/bike/searchAddedBike.dart';
...
class AddedBikeList extends StatefulWidget {
  String ds;
  AddedBikeList({super.key,required this.ds});
...
class _AddedBikeListState extends State<AddedBikeList> {
  ...
  /// TODO Upload Receipt
  Future _pickUploadBikeReceipt(ImageSource source) async {
    File selected = await ImagePicker.pickImage(source: source);
    setState(() {
      _receipt = selected;
    });
    final file = File(_receipt.path);
    final destination = "Bike_Receipts/${DateTime.now()}.png";
    if (_receipt != null) {
      Reference reference = FirebaseStorage.instance
          .ref(_auth.currentUser?.email)
          .child(destination);
  ...
  void addToLostBikeList(BuildContext context, DocumentSnapshot ds) {
    try {
      ...
      documentReference.set(lostBikeData).whenComplete(() {
        ...
        Map<String, dynamic> bikeReceiptData = ({
          "Bike_Receipt": receiptSnap,
          "Time": DateTime.now(),
        });

        receiptDocReference.set(bikeReceiptData).whenComplete(() {
      ...
      builder: (context) {
        return AlertDialog(
          ...
          title: Column(
            ...
            children: [
              Text("Model: $bikeName", style: viewOutputAlertDialogTextStyle()),
