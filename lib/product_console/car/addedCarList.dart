...
import 'package:verify_devices/product_console/car/searchAddedCar.dart';
...
class _AddedCarListState extends State<AddedCarList> {
  ...
  /// TODO Upload Receipt
  Future _pickUploadCarReceipt(ImageSource source) async {
    File selected = await ImagePicker.pickImage(source: source);
    setState(() {
      _receipt = selected;
    });
    final file = File(_receipt.path);
    final destination = "Car_Receipts/${DateTime.now()}.png";
    if (_receipt != null) {
      Reference reference = FirebaseStorage.instance
          .ref(_auth.currentUser!.email)
          .child(destination);
  ...
  void addToLostCarList(BuildContext context, DocumentSnapshot ds) {
    try {
      ...
      documentReference.set(lostCarData).whenComplete(() {
        ...
        Map<String, dynamic> carReceiptData = ({
          "Car_Receipt": receiptSnap,
          "Time": DateTime.now(),
        });

        receiptDocReference.set(carReceiptData).whenComplete(() {
